/*
 * @brief This file contains the CDC UART/Serial Port example using USB ROM Drivers.
 *
 * @note
...
void USB_CDC_Init(void)
{
    ...
    LOG_MSG(eDB_MSK_LIST_USB, "USB Init %s\r\n", (ret == 0 ? "Pass":"Fail"));
 ...
 * @return	Function should not exit.
 */
void utilFunc_Usb_Task(void *pParameters)
{
	USBD_API_INIT_PARAM_T usb_param;
	USB_CORE_DESCS_T desc;
	ErrorCode_t ret = LPC_OK;
	...
	while (1) {
		...
		if (prompt)
        {
            ...
            if (rdCnt)
            {
                Board_Uart_Write(eBOARD_UART_UART1, rdCnt, g_rxBuff, 0);
			}

            while(Board_Uart_Read(eBOARD_UART_UART1, 1, g_rxBuff, 0))
            {
				vcom_write(&g_rxBuff[0], 1);
            }
		}

        if(0 == utilCounterGet(eCOUNTER_TYPE_FAN_CHECK))
        {
            utilCounterSet(eCOUNTER_TYPE_FAN_CHECK, 1000);

            if(FALSE == Board_Gpio_Get(pSYS_FAN_OUT))
            {
                Board_Uart_Write(eBOARD_UART_UART1, 8, "SYS_FAN\n", 0);
...
StaticTask_t m_xUsbCdcTaskTCB;
/* Buffer that the task being created will use as its stack. */
StackType_t m_axUsbCdcStack[USB_CDC_TASK_STACK_SZ];

static BYTE m_cUsbEnable = FALSE;

void USB_CDC_Init(void)
{
    if(FALSE == m_cUsbEnable) {
        /* System Monitor thread */
        xTaskCreateStatic(utilFunc_Usb_Task,
                          "USB CDC Task",
                          USB_CDC_TASK_STACK_SZ,
                          NULL,
                          SB_CDC_TASK_PRIORITY,
                          m_axUsbCdcStack,
                          &m_xUsbCdcTaskTCB);
